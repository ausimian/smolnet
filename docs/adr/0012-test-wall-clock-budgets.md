...
Inflating the constants unconditionally was rejected. A budget that is generous
on every host stops reporting the regression it exists to catch, and the ring
test's margin is load-bearing: it has to cover retry timing, not just delivery.
Host-conditional scaling keeps the original strict local budget while giving a
contended shared runner the extra margin it needs.
...
- A **quiescence** budget bounds how long the test waits to conclude that
  ...
  `SmolNet.Test.Timing.quiescence(budget)` and are never scaled: changing a
  requested operation timeout would relax that scenario, while extending a
  silence check would slow the suite rather than help a healthy operation make
  progress.
...
against, keeps the strict bound. A fixed quiescence window also cannot prove
absence while its producer is descheduled: on a sufficiently contended runner,
an event that arrives after the window can produce a false pass. Scaling that
window would reduce but not eliminate this scheduler-dependent risk, so the
suite keeps the requested scenario and its running cost unchanged.
